        ...
        let worker = thread::spawn(move || {
            while !stopping.load(Ordering::Acquire) {
                ...
                let (status, body): (&str, &[u8]) = match path {
                    Some(route) if matches!(mode, Mode::All) && route.starts_with("/overlay/") => {
                        ("200 OK", &right)
                    }
                    Some(route) if matches!(mode, Mode::All) && route.ends_with(".png") => {
                        ("200 OK", &left)
                    }
                    ...
                    Some("/1/1/0.png") => match mode {
                        ...
                        Mode::InvalidRight => {
                            ("200 OK", b"this is an upstream HTML error, not an image")
                        }
                        Mode::All => ("200 OK", &right),
                    },
                    _ => ("404 Not Found", b""),
                };
...
async fn fetch(request: Request, base: &Path, output: &Path) -> Result<Manifest, String> {
    tokio::time::timeout(
        ...
        pipeline::fetch(request, base, output),
    )
    .await
    .expect("fixture job must terminate within 15 seconds")
}
...
fn assert_no_staging(parent: &Path) {
    for entry in fs::read_dir(parent).unwrap() {
        ...
        assert!(
            !name.to_string_lossy().starts_with(".geod-stage-"),
            "failed/published jobs must clean their stage directory"
        );
    }
}
...
async fn publishes_hashes_pixels_and_geotiff_mercator_tags() {
    ...
    let manifest = fetch(service.request(false), temp.path(), &output)
        .await
        .unwrap();
    ...
    let DecodingResult::U8(pixels) = decoder.read_image().unwrap() else {
        panic!("expected RGB8 GeoTIFF")
    };
    assert_eq!(&pixels[..3], &LEFT);
    assert_eq!(&pixels[256 * 3..256 * 3 + 3], &RIGHT);
    let preview = image::open(output.join("imagery-preview.png"))
        .unwrap()
        .to_rgb8();
    assert_eq!(
        preview
            .get_pixel(preview.width() / 4, preview.height() / 2)
            .0,
        LEFT
    );
    assert_eq!(
        preview
            .get_pixel(preview.width() * 3 / 4, preview.height() / 2)
            .0,
        RIGHT
    );
...
async fn missing_tiles_fail_without_publishing_a_partial_directory() {
    ...
    let error = fetch(service.request(false), temp.path(), &output)
        .await
        .unwrap_err();
    ...
    assert_eq!(
        ...
        2,
        "404 must not enter the long retry queue"
    );
    assert_no_staging(temp.path());
}
...
async fn explicit_partial_results_report_white_gaps_and_remain_inspectable() {
    ...
    let manifest = fetch(service.request(true), temp.path(), &output)
        .await
        .unwrap();
    ...
    assert_eq!(
        pipeline::inspect(&output).unwrap().quality.status,
        "partial"
    );
    let preview = image::open(output.join("imagery-preview.png"))
        .unwrap()
...
async fn http_success_with_invalid_image_bytes_is_not_a_complete_result() {
    ...
    let error = fetch(service.request(false), temp.path(), &output)
        ...
        .unwrap_err();
    assert!(
        error.contains("MISSING_TILES") || error.contains("INVALID_TILE"),
        "{error}"
    );
    assert!(!output.exists());
    assert_no_staging(temp.path());
...
async fn existing_output_is_preserved_before_any_http_acquisition() {
    ...
    assert_eq!(
        fs::read(output.join("user.txt")).unwrap(),
...
async fn inspect_detects_same_length_asset_corruption() {
    ...
    fetch(service.request(false), temp.path(), &output)
        .await
        .unwrap();
...
async fn mixed_bundle_preserves_water_and_tourism_roles_from_local_geojson() {
    ...
    request.vector = Some(
        serde_json::from_value(json!({"input":"input.geojson","layers":["water","tourism"]}))
            .unwrap(),
    );
    ...
    assert!(!manifest
        .layers
        .iter()
        .any(|layer| layer.id == "transportation"));
    ...
    let river = geojson["features"]
        .as_array()
        .unwrap()
        .iter()
        .find(|feature| feature["properties"]["name"] == "Fixture river")
        .unwrap();
...
#[tokio::test]
async fn multi_zoom_exports_match_desktop_raster_formats() {
    let _guard = JOBS.lock().await;
    let service = XyzFixture::start(Mode::All);
    let temp = tempfile::tempdir().unwrap();
    for format in ["geotiff", "mbtiles", "gpkg", "tiles"] {
        let mut request = service.request(false);
        let imagery = request.imagery.as_mut().unwrap();
        imagery.zoom = 0;
        imagery.zoom_levels = Some(vec![1, 0]);
        imagery.format = format.into();
        imagery.generate_sidecars = format == "geotiff";
        request.limits.max_tiles = 3;
        request.limits.max_pixels = 3 * 256 * 256;
        let plan = pipeline::plan(&request).unwrap();
        assert_eq!(plan["imagery"]["tileCount"], 3);
        assert_eq!(plan["imagery"]["zoomLevels"], json!([0, 1]));
        let output = temp.path().join(format);
        let manifest = fetch(request, temp.path(), &output).await.unwrap();
        assert_eq!(manifest.quality.status, "complete");
        assert!(output.join("imagery-preview.png").is_file());
        match format {
            "geotiff" => {
                for zoom in [0, 1] {
                    assert!(output.join(format!("imagery-z{zoom}.tif")).is_file());
                    assert!(output.join(format!("imagery-z{zoom}.tfw")).is_file());
                    assert!(output.join(format!("imagery-z{zoom}.prj")).is_file());
                }
            }
            "mbtiles" | "gpkg" => {
                let db =
                    rusqlite::Connection::open(output.join(format!("imagery.{format}"))).unwrap();
                let count: i64 = db
            ...
            "tiles" => {
                assert!(output.join("tiles/0/0/0.png").is_file());
                assert_eq!(
                    manifest
                        .assets
                        .iter()
...
#[tokio::test]
async fn overlay_composes_pixels_and_resume_reuses_valid_tiles() {
    let _guard = JOBS.lock().await;
    let service = XyzFixture::start(Mode::All);
    let temp = tempfile::tempdir().unwrap();
    let mut request = service.request(false);
    let imagery = request.imagery.as_mut().unwrap();
    imagery.format = "png".into();
    imagery.overlays = vec![serde_json::from_value(json!({
        "url":format!("http://{}/overlay/{{z}}/{{x}}/{{y}}.png",service.address),
        "source":"synthetic-overlay","attribution":"Fixture"
    }))
    .unwrap()];
    let work = temp.path().join("download-cache");
    let first = temp.path().join("first");
    let manifest = pipeline::fetch_with_work_dir(request, temp.path(), &first, Some(&work))
        .await
        .unwrap();
    assert_eq!(manifest.provenance.len(), 2);
    let image = image::open(first.join("imagery.png")).unwrap().to_rgb8();
    assert_eq!(image.get_pixel(10, 10).0, RIGHT);
    let requests = service.requests.load(Ordering::Relaxed);
    let mut request = service.request(false);
    request.imagery.as_mut().unwrap().format = "png".into();
    request.imagery.as_mut().unwrap().overlays = vec![serde_json::from_value(json!({
        "url":format!("http://{}/overlay/{{z}}/{{x}}/{{y}}.png",service.address),
        "source":"synthetic-overlay","attribution":"Fixture"
    }))
    .unwrap()];
    let second = temp.path().join("second");
    pipeline::fetch_with_work_dir(request, temp.path(), &second, Some(&work))
        .await
        .unwrap();
    assert_eq!(service.requests.load(Ordering::Relaxed), requests);
    pipeline::inspect(&second).unwrap();
    let mut wrong = service.request(false);
    wrong.imagery.as_mut().unwrap().zoom = 0;
    let error =
        pipeline::fetch_with_work_dir(wrong, temp.path(), &temp.path().join("wrong"), Some(&work))
            .await
            .unwrap_err();
    assert!(error.contains("WORK_DIR_MISMATCH"));
}

#[tokio::test]
async fn drawn_polygon_masks_png_without_vector_input() {
    let _guard = JOBS.lock().await;
    let service = XyzFixture::start(Mode::All);
    let temp = tempfile::tempdir().unwrap();
    let mut request = service.request(false);
    let imagery = request.imagery.as_mut().unwrap();
    imagery.format = "png".into();
    imagery.crop_to_shape = true;
    imagery.polygon = Some(
        serde_json::from_value(json!([[
            {"lat":5.0,"lng":-60.0},{"lat":5.0,"lng":-10.0},
            {"lat":50.0,"lng":-10.0},{"lat":50.0,"lng":-60.0}
        ]]))
        .unwrap(),
    );
    let output = temp.path().join("drawn-crop");
    let manifest = fetch(request, temp.path(), &output).await.unwrap();
    assert_eq!(manifest.quality.status, "complete");
    let raster = image::open(output.join("imagery.png")).unwrap().to_rgba8();
    assert!(raster.pixels().any(|pixel| pixel.0[3] == 255));
    assert!(raster.pixels().any(|pixel| pixel.0[3] == 0));
    pipeline::inspect(&output).unwrap();
}

#[tokio::test]
async fn geotiff_pyramid_uses_shared_bigtiff_writer() {
    let _guard = JOBS.lock().await;
    let service = XyzFixture::start(Mode::All);
    let temp = tempfile::tempdir().unwrap();
    let mut request = service.request(false);
    request.bounds = [-50.0, 1.0, 50.0, 2.0];
    request.imagery.as_mut().unwrap().zoom = 3;
    request.imagery.as_mut().unwrap().build_pyramid = true;
    request.limits.max_tiles = 8;
    request.limits.max_pixels = 8 * 256 * 256;
    let output = temp.path().join("pyramid");
    let manifest = fetch(request, temp.path(), &output).await.unwrap();
    let raster = fs::read(output.join("imagery.tif")).unwrap();
    assert_eq!(&raster[..4], b"II\x2b\x00");
    assert!(manifest
        .assets
        .iter()
        .any(|asset| asset.path == "imagery.tif"));
    pipeline::inspect(&output).unwrap();
}

#[test]
fn local_cli_can_opt_into_larger_limits_without_raising_hosted_limits() {
    let request: Request = serde_json::from_value(json!({
        "schemaVersion":"1.0","name":"bounds check","bounds":[-1.0,1.0,1.0,2.0],
        "imagery":{"url":"https://example.com/{z}/{x}/{y}.png",
            "source":"fixture","attribution":"fixture","zoom":1},
        "limits":{"maxTiles":5000,"maxPixels":300000000,"timeoutSeconds":7200}
    }))
    .unwrap();
    assert!(pipeline::plan(&request).is_err());
    assert!(pipeline::plan_local(&request).unwrap()["ok"]
        .as_bool()
        .unwrap());
}
