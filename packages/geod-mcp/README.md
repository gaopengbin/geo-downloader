...
> Connect the GeoD imagery download MCP to this agent. If you are running on my Windows x64 computer with Node.js 22+, install the [GeoD MCP 0.1.1 release package](https://github.com/gaopengbin/geo-downloader/releases/download/geod-mcp-v0.1.1/geod-mcp-0.1.1.tgz) in my current workspace with `npx --yes --package https://github.com/gaopengbin/geo-downloader/releases/download/geod-mcp-v0.1.1/geod-mcp-0.1.1.tgz geod-mcp install codex --package https://github.com/gaopengbin/geo-downloader/releases/download/geod-mcp-v0.1.1/geod-mcp-0.1.1.tgz --workspace "<absolute path of my workspace>"`; use `workbuddy` instead of `codex` in WorkBuddy. If you are cloud-hosted, add `https://laogao.xyz/geod-mcp/mcp` as a Streamable HTTP MCP server and let me complete my own account authorization in the browser. Do not ask for the publisher's computer path or token. After connecting, actually call `geod_capabilities` and `geod_plan` with an example request. Report the tool results or the exact blocker. This connection is for downloading imagery, not GeoStyle rendering.
...
```powershell
$pkg = 'https://github.com/gaopengbin/geo-downloader/releases/download/geod-mcp-v0.1.1/geod-mcp-0.1.1.tgz'
npx --yes --package $pkg geod-mcp install codex --package $pkg --workspace "C:\path\to\your\workspace"
npx --yes --package $pkg geod-mcp install workbuddy --package $pkg --workspace "C:\path\to\your\workspace"
```
...
The installer refuses to overwrite a different `geod` registration. The package is available on the [GitHub Release](https://github.com/gaopengbin/geo-downloader/releases/tag/geod-mcp-v0.1.1). npm publication of 0.1.1 is pending the publisher's two-factor confirmation.
