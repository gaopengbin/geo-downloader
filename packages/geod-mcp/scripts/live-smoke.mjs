...
const client = new Client({ name: 'geod-live-verification', version: '0.1.1' });
...
try {
  ...
  const imageResult = await call('geod_get_artifact', { jobId: fetched.jobId, artifactId: 'imagery-preview' });
  const image = imageResult.content.find(item => item.type === 'image');
  assert.equal(image?.mimeType, 'image/png');
  // Decode the actual downloaded preview returned over MCP.
  const imagePath = path.join(reportDir, 'sichuan-imagery-preview-via-mcp.png');
  await writeFile(imagePath, Buffer.from(image.data, 'base64'), { flag: 'wx' });
  assert.deepEqual(protocolErrors, []);
  const report = { ok: true, verifiedAt: new Date().toISOString(), protocol: 'official SDK 2.0.0 stdio Client', tools: tools.tools.map(t => t.name), timings: { ...timings, total: Date.now() - started }, imagePath, plan, fetched, protocolErrors, stderr: stderr.join('') };
  await writeFile(path.join(reportDir, 'verification.json'), JSON.stringify(report, null, 2), { flag: 'wx' });
  console.log(JSON.stringify({ ok: true, reportDir, imagePath, timings: report.timings, fetchJobId: fetched.jobId }));
