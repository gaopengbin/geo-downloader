...
1. Call `geod_capabilities` and `geod_sources` with `{ "action": "list" }` to see available sources. Local MCP shares GeoD CLI 0.3's current-user source registry: `register`, `update`, `remove`, `default` and single-tile `probe` are available. Use `imagery.sourceId` (also `imagery.overlays[].sourceId`) in a request, or set a local default. Read `geod://examples/sichuan` or `geod://examples/henan` for sample bounds. NASA Blue Marble is overview imagery, not current high-resolution satellite imagery.
...
| `geod_sources` | List sources; local MCP can register, update, remove, set a default and probe one tile |
