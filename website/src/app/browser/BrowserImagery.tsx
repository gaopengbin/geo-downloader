...
function planSummary(plan: ImageryPlan) {
  return { sourceId: plan.sourceId, source: plan.source, attribution: plan.attribution, bounds: plan.bounds, zoom: plan.zoom,
    tileCount: plan.tiles, width: plan.width, height: plan.height, execution: "当前浏览器直接从所选图源下载并拼接；GeoD 服务器不处理影像。" };
}
...
export default function BrowserImagery() {
  ...
  const [ready, setReady] = useState<{ preview: string; archive: string; source: string } | null>(null);
  ...
  const makePlan = useCallback((bounds: Bounds, level: number, sourceId: string) => planBrowserImagery(bounds, level, sourceById(sourceId)), [sourceById]);
  const toolPlan = useCallback((input: Record<string, unknown>) => {
    ...
    return makePlan(bounds as Bounds, input.zoom as number, typeof input.sourceId === "string" ? input.sourceId : activeRef.current);
  }, [makePlan]);
  ...
  const runFetch = useCallback(async (next: ImageryPlan, externalSignal?: AbortSignal) => {
    ...
    setValues(next.bounds.map(String)); setZoom(next.zoom); setPlan(next); setProgress(null); setMessage("正在当前浏览器中下载和拼接影像…");
    ...
    try {
      ...
      urlsRef.current = [preview, archive]; setReady({ preview, archive, source: next.source });
      setMessage("拼接完成。点击“保存数据包”把影像、定位文件和来源说明存到你的设备。");
  ...
  useEffect(() => {
    ...
    const tools: Tool[] = [
      { name: "geod_browser_capabilities", description: "查看当前页面的本机影像下载能力；GeoD 服务器不处理影像。",
        ...
        execute: async () => result({ maxTiles: 256, maxPixels: 16_777_216, output: "PNG + EPSG:3857 PGW/PRJ + manifest.json，打包为 ZIP", requiresOpenPage: true, sources: "geod_browser_sources；用户图源保存在当前浏览器，需支持 CORS" }) },
      ...
      { name: "geod_browser_plan", description: "按 WGS84 范围、级别和可选 sourceId 估算当前浏览器影像下载；不发起网络下载。先列出图源。",
        inputSchema: requestSchema, annotations: { readOnlyHint: true }, execute: async input => result(planSummary(toolPlan(input))) },
      { name: "geod_browser_fetch", description: "仅在用户要求下载时调用。当前浏览器直接从所选图源下载拼接，完成后让用户保存 ZIP。不要用来测试连接。",
  ...
  const currentPlan = () => makePlan(values.map(Number) as Bounds, zoom, activeId);
  const handlePlan = () => { try { const next = currentPlan(); setPlan(next); setMessage(`范围有效：${next.tiles} 块瓦片，输出约 ${next.width} × ${next.height} 像素。`); }
  ...
  return <div className={styles.workspace}>
    ...
    <section className={styles.resultCard} aria-labelledby="browser-result-title">
      ...
      {ready ? <><div className={styles.preview}><img src={ready.preview} alt={`当前浏览器拼接的 ${ready.source} 影像预览`} /></div>
        ...
        <p className={styles.finePrint}>ZIP 包含 imagery.png、imagery.pgw、imagery.prj 和 manifest.json。文件不会经过 GeoD 服务器。</p></>
        : <div className={styles.empty}>{plan ? <><strong>{plan.tiles} 块瓦片</strong><span>{plan.width} × {plan.height} 像素 · RGBA 内存约 {Math.ceil(plan.width * plan.height * 4 / 1024 / 1024)} MiB</span></>
