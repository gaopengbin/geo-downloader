...
1. Call `geod_capabilities` to learn the available sources, limits, and examples. If the user has not supplied a complete request, read a relevant `geod://examples/henan` or `geod://examples/sichuan` resource and adapt it to the requested area and source. Bounds are WGS84 `[west, south, east, north]`; do not invent a precise boundary or claim a source is permitted without checking.
2. Call `geod_plan` with `{ "request": ... }` to validate the request and estimate tiles, pixels, and footprint. Planning does not download anything. Explain the plan and any source or size limitation in plain language.
...
The bundled NASA Blue Marble examples are overview imagery, not current high-resolution satellite imagery. A prepared administrative boundary is not a verified tourism or POI dataset. GeoD MCP does not render GeoStyle maps.
