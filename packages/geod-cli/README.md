...
```powershell
npm install --global geod-cli@0.3.0
geod --version
geod --help
```
...
See the [GeoD CLI 0.3 guide](https://github.com/gaopengbin/geo-downloader/blob/geod-cli-v0.3.0/docs/geod-cli-0.3.md) for request examples and limits.
...
For maintainers, run `node scripts/package-geod-cli-npm.mjs` from the repository. The resulting tarball is staged under `output/geod-cli-public-0.3.0/npm/`; the executable is not checked into the package source directory. Direct packing without the verified executable fails.
