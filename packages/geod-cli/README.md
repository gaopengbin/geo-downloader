...
```powershell
npm install --global geod-cli@0.1.1
geod --version
geod --help
```
...
For maintainers, run `node scripts/package-geod-cli-npm.mjs` from the repository. The resulting tarball is staged under `output/geod-cli-public-0.1.1/npm/`; the executable is not checked into the package source directory. Direct packing without the verified executable fails.
